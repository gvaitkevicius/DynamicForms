...
{
    public class V_DISPONIBILIDADE_FACA_MAP : IEntityTypeConfiguration<V_DISPONIBILIDADE_FACA>
    {
        public void Configure(EntityTypeBuilder<V_DISPONIBILIDADE_FACA> builder)
        {
            builder.ToTable("V_DISPONIBILIDADE_FACA");
            ...
            builder.HasOne(x => x.ProdutoFaca).WithMany(x => x.Indisponibilidade).HasForeignKey(x => x.PRO_ID);
            builder.HasOne(x => x.Calendario).WithMany(x => x.IndisponibilidadeFaca).HasForeignKey(x => x.CAL_ID);
